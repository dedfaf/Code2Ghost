...
async function getPost(context) {
	...
	if (posts) {
		...
		if (selectedPostTitle) {
			...
			// console.log(selectedPost.created_at, selectedPost.updated_at, selectedPost.published_at);
			...
			if (frontMatter.status == `published`) {
				frontMatterString = matter.stringify('', frontMatter).replace(/---\s*$/, comment + '---');
			} else {
				frontMatterString = matter.stringify('', frontMatter).replace(/---\s*$/, comment + commentPublish + '---');
			}
